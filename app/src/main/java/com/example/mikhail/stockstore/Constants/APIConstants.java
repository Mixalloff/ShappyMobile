package com.example.mikhail.stockstore.Constants;

/**
 * Created by mikhail on 19.12.15.
 */
public class APIConstants {

    public final static String USER_AUTH = "userauth";
    public final static String USER_REGISTER = "register";
    public final static String USER_SUBSCRIBE_STOCK = "useraddstock";
    public final static String USER_UNSUBSCRIBE_STOCK = "userdeletestock";
    public final static String USER_GET_FEED = "usergetfeed";
    public final static String GET_ALL_STOCKS = "getallstocks";
    public final static String GET_ALL_COMPANIES = "getallcompanies";
    public final static String GET_ALL_CATEGORIES = "getallcategories";
    public final static String USER_GET_STOCKS_INFO = "getstocksinfo";
    public final static String USER_GET_STOCKS_BY_COMPANY = "getstocksbycompany";
    public final static String USER_GET_STOCKS_BY_WORDPATH = "getstocksbyword";
    public final static String USER_GET_STOCKS_BY_FILTER = "getStocksByFilter";
    public final static String USER_GET_ALL_FRIENDS = "getallfriends";
    public final static String USER_ADD_FRIEND = "addfriend";
    public final static String USER_DELETE_FRIEND = "deletefriend";
    public final static String USER_GET_FRIENDS_FEED = "friendsfeed";

    public final static String USER_GET_FRIENDS_FILTER = "friendsfilter";

    public final static String USER_AUTH_ROUTE = "user/authorize";
    public final static String USER_REGISTER_ROUTE = "user/register";
    public final static String GET_ALL_STOCKS_ROUTE = "user/stocks/all";
    public final static String USER_SUBSCRIBE_STOCK_ROUTE = "user/stocks/subscribe";
    public final static String USER_UNSUBSCRIBE_STOCK_ROUTE = "user/stocks/unsubscribe";
    public final static String USER_GET_FEED_ROUTE = "user/stocks/feed";
    public final static String GET_ALL_COMPANIES_ROUTE = "user/companies/all";
    public final static String GET_ALL_CATEGORIES_ROUTE = "user/categories/all";
    public final static String USER_GET_STOCKS_INFO_ROUTE = "user/stocks/info";
    public final static String USER_GET_STOCKS_BY_COMPANY_ROUTE = "user/stocks/filter/company";
    public final static String USER_GET_STOCKS_BY_WORDPATH_ROUTE = "user/stocks/filter/search";
    public final static String USER_GET_STOCKS_BY_FILTER_ROUTE = "user/stocks/filter";
    public final static String USER_GET_ALL_FRIENDS_ROUTE = "user/friends/all";
    public final static String USER_ADD_FRIEND_ROUTE = "user/friends/add";
    public final static String USER_DELETE_FRIEND_ROUTE = "user/friends/delete";
    public final static String USER_GET_FRIENDS_FEED_ROUTE = "user/stocks/filter/friends";

    public final static String USER_GET_FRIENDS_FILTER_ROUTE = "user/friends/filter";

}